		...
		<div className="relative min-h-screen text-white overflow-hidden">
			...
			<motion.div
				...
				animate={{
					opacity: 1,
					background: [
						"linear-gradient(135deg, #00111a, #00334c)",
						"linear-gradient(135deg, #002b3a, #005f7a)",
						"linear-gradient(135deg, #00111a, #00334c)",
					],
				}}
				transition={{
			...
			<div className="absolute inset-0 z-0 pointer-events-none">
				<div className="w-full h-full bg-[radial-gradient(circle,_rgba(255,255,255,0.05)_1px,_transparent_1px)] bg-[length:50px_50px] opacity-5" />
			</div>
			...
			<div className="relative z-10 container mx-auto px-4 py-16">
				...
				<div className="p-6">
					<h1 className="text-2xl font-bold mb-6">Wallet Query</h1>
							...
							<div>
								...
								<div className="flex items-center space-x-2 mb-4">
									<input
										...
										className="px-4 py-2 rounded bg-neutral-800 text-gray-200 border border-neutral-700 w-full"
									/>
									<button
										onClick={handleSearch}
										className="bg-gradient-to-r from-[#14F195] to-[#00D1FF] text-black px-4 py-2 rounded-full hover:opacity-80 transition-all shadow-lg font-bold"
									>
										Search
									</button>
								</div>
								...
								{filteredTransactions.length === 0 ? (
									...
									<AnimatePresence>
										{filteredTransactions.map((tx, idx) => (
											<motion.div
												key={idx}
												className="bg-neutral-800 p-4 rounded shadow-sm relative overflow-hidden group"
											...
											>
												...
												>
													<motion.div
														...
														className="absolute top-0 left-0 h-full w-1/3 bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent"
													/>
												</motion.div>

												{/* Neon hover outline */}
												<div className="absolute inset-0 rounded group-hover:shadow-[0_0_15px_#00ffffcc] transition-shadow duration-300 pointer-events-none"></div>

												<div className="flex items-center space-x-3 relative z-10">
													{tx.tokenProperty.metadata?.image && (
														<Image
															src={tx.tokenProperty.metadata.image}
															alt="Transaction"
															width={48}
															height={48}
															className="rounded"
													...
													<div>
														...
														<p className="text-base">
															<span className="font-medium">
																Sent {tx.tokenProperty.amount} {tx.tokenProperty.symbol} ({tx.tokenProperty.name})
															</span>{" "}
															To {tx.to}
														</p>
