...
pub const AGENT_API_URL: &str = "http://localhost:8000/query/generate-query";
...
impl SWqueryClient {
    ...
    pub async fn query(&self, input: &str, pubkey: &str) -> Result<Value, SdkError> {
        ...
        println!("request above as curl: curl -X POST -H \"x-api-key: {}\" -H \"Content-Type: application/json\" -d '{}' {}", &self.api_key, &payload, AGENT_API_URL);
        ...
        fn to_value_response<T: serde::Serialize>(
            response: T,
    ...
    pub async fn get_highest_snapshot_slot(
        &self,
    ...
    pub async fn get_max_shred_insert_slot(
        &self,
